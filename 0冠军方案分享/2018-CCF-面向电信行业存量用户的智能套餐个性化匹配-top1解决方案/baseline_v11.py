...
train_p =  pd.read_csv('val_prob_hebing2.csv')
train = train.merge(train_p,on='user_id',how='left')
...
train_total_fee = pd.read_csv('./sub/val_fee_pred.csv')
train_total_fee.columns = ['user_id', 'fee_pred']
test_total_fee.columns = ['user_id', 'fee_pred']
train = train.merge(train_total_fee,on='user_id',how='left')
test  = test.merge(test_total_fee,on='user_id',how='left')
...
train = one_hot_encoder(train,'online_time',n=2000,nan_as_category=True)
train = one_hot_encoder(train,'contract_time',n=1000,nan_as_category=True) 
...
def kfold_lightgbm(params,df, predictors,target,num_folds, stratified = True,
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df[target])):
        ...
        #weight_num =  train_df['weight'].iloc[train_idx]
...
#        xgtrain.set_weight(np.array(weight_num))
...
x = []
no_use = ['current_service', 'user_id','group','weight'
